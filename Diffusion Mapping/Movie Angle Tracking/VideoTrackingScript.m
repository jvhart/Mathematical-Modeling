...
% diffusion mapping in the context of a video tracking the movement of an
% object from differnt viewing angles.  The subject of the pictures is a KU 
% can cozy from various angles.  Each of the frames is independently
...
% V=VideoReader('Jayhawk Videos/JayhawkVideo01.mov');
V=VideoReader('Jayhawk Videos/JayhawkVideo02.mov');
...
Kfun=@(x,y) exp(-sum(abs(x-y))/(300*num_pixels));
K=GenerateK(X,Kfun);
